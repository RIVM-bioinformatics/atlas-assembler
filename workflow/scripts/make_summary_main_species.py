...
def parse_skani_results(skani_output_path: pathlib.Path) -> pd.DataFrame:
    ...
    # Only keep columns that exist in the DataFrame because I was getting an error when only one hit was found
    column_order_existing = [col for col in column_order if col in top_hits_wide.columns]
    top_hits_wide = top_hits_wide[column_order_existing].reset_index()
    # top_hits_wide = top_hits_wide.drop(columns=["skani_1_ref_file", "skani_1_query_file", "skani_2_ref_file", "skani_2_query_file"])
    cols_to_drop = [c for c in ["skani_1_ref_file", "skani_1_query_file", "skani_2_ref_file", "skani_2_query_file"] if c in top_hits_wide.columns]
    top_hits_wide = top_hits_wide.drop(columns=cols_to_drop)
