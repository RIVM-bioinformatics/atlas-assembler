...
rule checkm:
    ...
    output:
        result=OUT + "/qc_consensus_assembly/checkm/per_sample/{sample}/checkm_{sample}.tsv",
        tmp_dir1=directory(OUT + "/qc_consensus_assembly/checkm/per_sample/{sample}/bins"),
        tmp_dir2=directory(OUT + "/qc_consensus_assembly/checkm/per_sample/{sample}/storage"),
    message:
        "Running CheckM for {wildcards.sample}."
    conda:
        "../../envs/checkm.yaml"
    ...
    shell:
        ...
        if [ "$(cat {input.flag_filtered})" = "sufficient" ]; then
            checkm taxonomy_wf genus "$(<{input.selected_genus})" \
                {params.input_dir} \
                {params.output_dir} \
                -t {threads} \
