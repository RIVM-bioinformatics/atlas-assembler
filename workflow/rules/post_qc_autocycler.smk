rule run_subsets_quast:
    # input:
    #     # assembly=lambda wildcards: OUT + f"/medaka/{wildcards.sample}/flye/assembly.fasta"
    #     subset_assembly = expand(OUT + "/autocycler/{sample}/assemblies/{assembler}_{sample}_{subset}.fasta", sample=SAMPLES, assembler = assembler_list, subset = subsets_used)
    # output:
    #     report=OUT + "/qc_subset_assembly/quast/report.tsv",
    #     quast=OUT + "/qc_subset_assembly/quast/transposed_report.tsv"
    # message:
    ...
    #     output_dir=OUT + "/qc_subset_assembly/quast",
    # log:
    #     OUT + "/log/qc_subset_assembly/quast.log"
    # shell:
    #     """
    #     quast.py --threads {threads} {input.subset_assembly} --output-dir {params.output_dir} > {log}
    #     """
...
#     output:
#         # OUT + directory("/busco/{sample}/")
#         busco_out = directory(OUT + "/qc_subset_assembly/busco/{sample}/"),
#         short_summary = OUT + "/qc_subset_assembly/busco/{sample}/busco_results_{sample}/short_summary.specific.bacteria_odb10.busco_results_{sample}.txt"
#     message:
#         "Running BUSCO for {wildcards.sample}"
#     conda:
#         "../../envs/busco.yaml"
#     log:
#         OUT + "/log/qc_subset_assembly/busco/{sample}/busco.out"
#     params:
#         mode = "genome",
#         lineage = "bacteria_odb10",
#         short_summary_filename = "busco_results_{sample}",
...
#     shell:
#         """
#         busco -i {input.subset_assembly} --out_path {output.busco_out} -l {params.lineage} -o {params.short_summary_filename}\
#         -m {params.mode} -f 
#         """
